...
class NexiaThermostat:
    ...
    def __init__(self, device_id, account_id, thermostat_id):
        ...
        self.logger.threaddebug(f"{dev.name}: NexiaThermostat __init__ starting, pluginProps =\n{dev.pluginProps}")

        return

    def update(self):
        dev = indigo.devices[self.device_id]
        self.logger.threaddebug(f"{dev.name}: Update")
        ...
        if not account_dev.states['authenticated']:
            self.logger.info(f'Nexia account not authenticated yet; not initializing state of device {self.address}')
            return

        if not self.account:
            try:
                self.account = indigo.activePlugin.nexia_accounts[self.account_id]
                self.logger.debug(f"{dev.name}: Nexia Account device assigned, {self.account_id}")
            except (Exception, ):
                self.logger.error("updatable: Error obtaining Nexia account object")
                return

        update_list = [{'key': "thermostat_name", 'value': self.account.get_thermostat_name(self.thermostat_id)},
                       {'key': "thermostat_model", 'value': self.account.get_thermostat_model(self.thermostat_id)},
                       {'key': "thermostat_firmware", 'value': self.account.get_thermostat_firmware(self.thermostat_id)},
                       {'key': "thermostat_type", 'value': self.account.get_thermostat_type(self.thermostat_id)},
                       {'key': "fan_mode", 'value': self.account.get_fan_mode(self.thermostat_id)},
                       {'key': "outdoor_temperature", 'value': self.account.get_outdoor_temperature(self.thermostat_id)},
                       {'key': "dehumidify_setpoint", 'value': self.account.get_dehumidify_setpoint(self.thermostat_id)},
                       {'key': "system_status", 'value': self.account.get_system_status(self.thermostat_id)},
                       {'key': "air_cleaner_mode", 'value': self.account.get_air_cleaner_mode(self.thermostat_id)},
                       {'key': "is_blower_active", 'value': self.account.is_blower_active(self.thermostat_id)}]
...
class NexiaZone:
    ...
    def __init__(self, device_id, account_id, thermostat_id, zone_id):
        ...
        self.logger.threaddebug(f"{dev.name}: NexiaZone __init__ starting, pluginProps =\n{dev.pluginProps}")
    ...
    def update(self):
        ...
        account_dev = indigo.devices[self.account_id]
        if not account_dev.states['authenticated']:
            self.logger.info(f'{dev.name}: Nexia account {self.account_id} not authenticated yet; not updating device')
            return
