...
class NexiaThermostat:
    ...
    def _get_url(self, url):
        ...
        try:
            ...
            self._check_response(f"Error getting url: {e}", request_url)
            return None
    ...
    def _get_thermostat_json(self, thermostat_id=None, force_update=False):
        ...
        with self.mutex:
            if self.thermostat_json is None or self._needs_update() or force_update is True:
                ...
                if request and request.status_code == 200:
                    ...
                    self._check_response("Failed to get thermostat JSON, session probably timed out", request)
        ...
        if thermostat_id is not None:
            ...
            raise KeyError(f"Thermostat ID {thermostat_id} does not exist. Available IDs: {thermostat_json_dict.keys()}")
    ...
    def _get_thermostat_put_url(self, text=None, thermostat_id=None):
        ...
        if thermostat_id is None and len(self.get_thermostat_ids()) == 1:
            ...
            raise KeyError(f"Thermostat ID {thermostat_id} does not exist. Available IDs: {self.get_thermostat_ids()}")
        return f"/houses/{str(self.house_id)}/xxl_thermostats/{str(thermostat_id)}/{text}"
        ...
        if GLOBAL_LOGIN_ATTEMPTS_LEFT > 0:
            ...
            if token:
                ...
                if request.url == self.AUTH_FORGOTTEN_PASSWORD_STRING:
                    raise Exception(f"Failed to login, getting redirected to {request.url}. Try to login manually on the website.")

            else:
                raise Exception("Failed to get csrf token")
        else:
            raise Exception(f"Failed to login after {GLOBAL_LOGIN_ATTEMPTS} attempts! Any more attempts may lock your account!")
    ...
    def set_fan_mode(self, fan_mode, thermostat_id=None):
        """
        Sets the fan mode.
    ...
    def set_air_cleaner(self, air_cleaner_mode, thermostat_id):
        """
    ...
    def get_zone_name(self, thermostat_id=None, zone_id=0):
        ...
        :return: str
        """
        return str(self._get_zone_key("name", thermostat_id=thermostat_id, zone_id=zone_id))
    ...
    def get_zone_current_mode(self, thermostat_id=None, zone_id=0):
        ...
        :return: str
        """
        return str(self._get_zone_key("last_zone_mode", thermostat_id=thermostat_id, zone_id=zone_id).upper())
    ...
    def get_zone_preset_setpoints(self, preset, thermostat_id=None, zone_id=0):
        ...
        if preset != self.PRESET_MODE_NONE:
            ...
            return (
                self._get_zone_key("preset_cool{}".format(index), thermostat_id=thermostat_id, zone_id=zone_id),
                self._get_zone_key("preset_heat{}".format(index), thermostat_id=thermostat_id, zone_id=zone_id)
            )
    ...
    def is_zone_calling(self, thermostat_id=None, zone_id=0):
        ...
        :return: bool
        """
        return self._get_zone_key("on_off_code", thermostat_id=thermostat_id, zone_id=zone_id) == "CALL"
    ...
    def check_heat_cool_setpoints(self, heat_temperature=None, cool_temperature=None, thermostat_id=None):
        ...
        if heat_temperature is not None and \
            ...
            raise AttributeError(f"The heat setpoint ({heat_temperature}) must be less than the cool setpoint ({cool_temperature}).")
        if heat_temperature is not None and \
                cool_temperature is not None and \
        ...
        if heat_temperature is not None and \
            ...
            raise AttributeError(f"The heat setpoint ({heat_temperature} must be less than the maximum temperature of {max_temperature} degrees.")
        if cool_temperature is not None and \
                not cool_temperature >= min_temperature:
            raise AttributeError(f"The cool setpoint ({cool_temperature}) must be greater than the minimum temperature of {min_temperature} degrees.")
    ...
    def is_zone_in_permanent_hold(self, thermostat_id=None, zone_id=0):
        ...
        :return: bool
        """
        return self._get_zone_key("permanent_hold", thermostat_id=thermostat_id, zone_id=zone_id)
    ...
    def call_temporary_hold(self, heat_temperature=None, cool_temperature=None, holdtime=0, thermostat_id=None, zone_id=0):
        """
    ...
    def set_zone_heat_cool_temp(self, heat_temperature=None, cool_temperature=None, set_temperature=None, thermostat_id=None, zone_id=0):
        """
    ...
    def round_temp(self, temperature, thermostat_id=None):
        """
