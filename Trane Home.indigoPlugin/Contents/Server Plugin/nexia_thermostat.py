...
class NexiaThermostat:
        ...
        if request is None or request.status_code != 200:
            if request is not None:
                ...
                for key in request.__attrs__:
                    response += "  {}: {}\n".format(key, getattr(request, key))
                raise Exception("{}\n{}".format(error_text, response))
            raise Exception("No response from session. {}".format(error_text))
    ...
    def _get_thermostat_json(self, thermostat_id=None, force_update=False):
        ...
        if thermostat_id is not None:
            ...
            raise KeyError(
                "Thermostat ID {} does not exist. Available IDs: {}".format(thermostat_id, thermostat_json_dict.keys()))
    ...
    def _get_thermostat_key(self, key, thermostat_id=None):
        ...
        raise KeyError("Key \"{}\" not in the thermostat JSON!".format(key))
    ...
    def _get_thermostat_put_url(self, text=None, thermostat_id=None):
        ...
        if thermostat_id is None and len(self.get_thermostat_ids()) == 1:
            ...
            raise KeyError(
                "Thermostat ID {} does not exist. Available IDs: {}".format(thermostat_id, self.get_thermostat_ids()))
        return "/houses/{}/xxl_thermostats/{}/{}".format(str(self.house_id), str(thermostat_id), text)
    ...
    def _get_zone_json(self, thermostat_id=None, zone_id=0):
        ...
        raise IndexError(
            "The zone_id ({}) does not exist in the thermostat zones.".format(zone_id))
    ...
    def _get_zone_key(self, key, thermostat_id=None, zone_id=0):
        ...
        raise KeyError("Zone {} key \"{}\" invalid.".format(zone_id, key))
        ...
        if GLOBAL_LOGIN_ATTEMPTS_LEFT > 0:
            ...
            if token:
                ...
                if request.url == self.AUTH_FORGOTTEN_PASSWORD_STRING:
                    raise Exception(
                        "Failed to login, getting redirected to {}. Try to login manually on the website.".format(request.url)
                        )
            ...
            raise Exception(
                "Failed to login after {} attempts! Any more attempts may lock your account!".format(GLOBAL_LOGIN_ATTEMPTS)
                )
    ...
    def set_dehumidify_setpoint(self, dehumidify_setpoint, thermostat_id):
        ...
        if self.has_relative_humidity(thermostat_id):
            ...
            if min_humidity <= dehumidify_setpoint <= max_humidity:
                ...
                raise ValueError(
                    "humidity_level out of range ({} - {})".format(min_humidity, max_humidity))
        else:
            raise Exception(
    ...
    def get_zone_preset_setpoints(self, preset, thermostat_id=None, zone_id=0):
        ...
        if preset != self.PRESET_MODE_NONE:
            ...
            return (
                self._get_zone_key("preset_cool{}".format(index),
                                   thermostat_id=thermostat_id,
                                   zone_id=zone_id),
                self._get_zone_key("preset_heat{}".format(index),
                                   thermostat_id=thermostat_id,
                                   zone_id=zone_id)
            )
        raise KeyError(
            "'{}' preset mode does not have any preset values.".format(self.PRESET_MODE_NONE))
    ...
    def check_heat_cool_setpoints(self, heat_temperature=None,
        ...
        if heat_temperature is not None and \
            ...
            raise AttributeError(
                "The heat setpoint ({}) must be less than the cool setpoint ({}).".format(heat_temperature, cool_temperature))
        if heat_temperature is not None and \
                cool_temperature is not None and \
                not cool_temperature - heat_temperature >= deadband:
            raise AttributeError(
                "The heat and cool setpoints must be at least {} degrees different.".format(deadband))
        if heat_temperature is not None and \
                not heat_temperature <= max_temperature:
            raise AttributeError(
                "The heat setpoint ({} must be less than the maximum temperature of {} degrees.".format(heat_temperature, max_temperature))
        if cool_temperature is not None and \
                not cool_temperature >= min_temperature:
            raise AttributeError(
                "The cool setpoint ({}) must be greater than the minimum temperature of {} degrees.".format(cool_temperature, min_temperature))
    ...
    def set_zone_mode(self, mode, thermostat_id=None, zone_id=0):
        ...
        if mode in self.OPERATION_MODES:
            ...
            raise KeyError(
                "Invalid mode \"{}\". Select one of the following: {}".format(mode, self.OPERATION_MODES))
